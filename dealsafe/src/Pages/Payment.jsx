...
function Payment() {
  ...
  const startPayment = async (event) => {
    ...
    const externalref = `DS-${transaction.id || transaction.documentId}-${Date.now()}`;
    try {
      const response = await fetch("/.netlify/functions/moolre-payment", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "initiate", payer: normalizePhone(payer), channel, amount: transaction.amount, externalref, reference: `DealSafe ${transaction.id || transaction.documentId}` }) });
      const result = await response.json();
      if (!response.ok || String(result.status) === "0") throw new Error(result.message || result.error || "Moolre could not start the payment.");
      setPaymentReference(externalref); setMessage(result.code === "200_OTP_REQ" ? "Moolre sent an OTP. Complete the prompt, then check payment status." : "Payment prompt sent. Approve it on the payer's phone, then check payment status.");
