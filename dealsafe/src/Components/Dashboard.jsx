...
function Dashboard() {
  ...
  useEffect(() => { if (!auth.currentUser) { setLoadingTransactions(false); return undefined; } const unsubscribe = onSnapshot(query(collection(db, "deals"), where("creatorId", "==", auth.currentUser.uid)), (snapshot) => { setTransactions(snapshot.docs.map((document) => { const data = document.data(); const statusLabels = { awaiting_seller: "Waiting for Seller", awaiting_buyer: "Waiting for Buyer", waiting_for_payment: "Waiting for Payment", payment_secured: "Payment Secured", awaiting_delivery: "Awaiting Delivery", awaiting_confirmation: "Awaiting Confirmation", ready_for_release: "Ready for Release", payment_released: "Payment Released", completed: "Completed", disputed: "Disputed", cancelled: "Cancelled", refunded: "Refunded" }; return { ...data, documentId: document.id, id: data.transactionCode || `DS-${document.id.slice(0, 5).toUpperCase()}`, name: data.title || "Untitled deal", role: data.creatorRole === "seller" ? "Seller" : "Buyer", otherParty: typeof data.otherParty === "object" ? data.otherParty?.fullName || "Other party" : data.otherParty || "Other party", amount: `GHS ${Number(data.amount || 0).toLocaleString("en-GH", { minimumFractionDigits: 2 })}`, status: statusLabels[data.status] || data.status || "Awaiting review" }; })); setLoadingTransactions(false); }, () => setLoadingTransactions(false)); return unsubscribe; }, []);
  useEffect(() => { if (!auth.currentUser) return undefined; const unsubscribe = onSnapshot(query(collection(db, "deals"), where("acceptedBy", "==", auth.currentUser.uid)), (snapshot) => { const statusLabels = { awaiting_seller: "Waiting for Seller", awaiting_buyer: "Waiting for Buyer", waiting_for_payment: "Waiting for Payment", payment_secured: "Payment Secured", awaiting_delivery: "Awaiting Delivery", awaiting_confirmation: "Awaiting Confirmation", ready_for_release: "Ready for Release", payment_released: "Payment Released", completed: "Completed", disputed: "Disputed", cancelled: "Cancelled", refunded: "Refunded" }; const acceptedTransactions = snapshot.docs.map((document) => { const data = document.data(); return { ...data, documentId: document.id, id: data.transactionCode || `DS-${document.id.slice(0, 5).toUpperCase()}`, name: data.title || "Untitled deal", role: data.creatorRole === "seller" ? "Seller" : "Buyer", otherParty: typeof data.otherParty === "object" ? data.otherParty?.fullName || "Other party" : data.otherParty || "Other party", amount: `GHS ${Number(data.amount || 0).toLocaleString("en-GH", { minimumFractionDigits: 2 })}`, status: statusLabels[data.status] || data.status || "Awaiting review" }; }); setTransactions((current) => [...current.filter((item) => !acceptedTransactions.some((accepted) => accepted.documentId === item.documentId)), ...acceptedTransactions]); }, () => {}); return unsubscribe; }, []);
